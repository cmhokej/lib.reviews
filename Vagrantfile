...
# run 'vagrant ssh', then run 'cd /vagrant && npm start'.
#
Vagrant.configure('2') do |config|
  config.vm.box = 'debian/jessie64'

  config.vm.post_up_message = <<-END
    To start lib.reviews, run 'vagrant ssh', then 'cd /vagrant && npm start'.
    lib.reviews will then run on http://localhost:8080."
  END
